import 'app_localizations.dart';

/// The translations for Turkish (`tr`).
class L10nTr extends L10n {
  L10nTr([String locale = 'tr']) : super(locale);

  @override
  String get languageSelect => 'Türkçe';

  @override
  String get next => 'Sonraki';

  @override
  String get takeStepWorldMeals => 'Yemeklerin Büyülü Dünyasına Adım Atın';

  @override
  String get awaitYou => 'Binlerce Nefis Tarif Sizi Bekliyor';

  @override
  String get clickBegin => 'Yemek Yapmaya Başlamak İçin Şimdi Tıklayın';

  @override
  String get signIn => 'Giriş Yapın';

  @override
  String get dontHaveAccount => 'Hesabınız Yok mu?';

  @override
  String get signUp => 'Kayıt Ol';

  @override
  String get email => 'Eposta';

  @override
  String get validEmailMessage => 'Lütfen Geçerli Bir Mail Adresi Giriniz';

  @override
  String get validPasswordMessage => 'Lütfen Geçerli Bir Şifre Giriniz';

  @override
  String get password => 'Şifre';

  @override
  String get doHaveAccount => 'Hesabınız Var mı?';

  @override
  String get nameMessage => 'Lütfen İsim Giriniz';

  @override
  String get name => 'İsim';

  @override
  String get surname => 'Soyisim';

  @override
  String get surnameMessage => 'Lütfen Soyisim Giriniz';

  @override
  String get exitMessage => 'Çıkış yapılıyor';

  @override
  String get profile => 'Profil';

  @override
  String get resetPassword => 'Şifre Sıfırlama';

  @override
  String get notifications => 'Bildirimler';

  @override
  String get theme => 'Tema';

  @override
  String get settings => 'Ayarlar';

  @override
  String get language => 'Dil';

  @override
  String get successLoginMessage => 'Anasayfaya Yönlendiriliyorsunuz';

  @override
  String get inCorrectEmailFormatMessage => 'Eposta Formatı Hatalı';

  @override
  String get inCorrectEmailPasswordMessage => 'Eposta ya da Şifre Hatalı';

  @override
  String get about => 'Hakkında';

  @override
  String get privacy => 'Gizlilik';

  @override
  String get categories => 'Kategoriler';

  @override
  String get myFavorites => 'Favorilerim';

  @override
  String get recipeDetails => 'Tarif Detayları';

  @override
  String get iRecipe => 'iRecipe';

  @override
  String get myProfile => 'Profilim';

  @override
  String get favorites => 'Favoriler';

  @override
  String get welcome => 'Hoşgeldin';

  @override
  String get myRecipe => 'Tariflerim';

  @override
  String get personalInfo => 'Kişisel Bilgiler';

  @override
  String get home => 'Anasayfa';

  @override
  String get add => 'Ekle';

  @override
  String get materials => 'Malzemeler';

  @override
  String get preparation => 'Hazırlanışı';

  @override
  String get foodName => 'Yemek İsmi';

  @override
  String get foodNameMessage => 'Lütfen Yemek İsmi Giriniz';

  @override
  String get materialsMessage => 'Lütfen Malzemeleri Giriniz';

  @override
  String get preparationMessage => 'Lütfen Hazırlanışını Giriniz';

  @override
  String get imageLinkMessage => 'Lütfen Resim Linki Giriniz';

  @override
  String get imageLink => 'Resim Linki';

  @override
  String get exampleLink => 'https://örnek.com/görsel';

  @override
  String get aboutDescription => 'Hoş Geldiniz iRecipe Ailesi! \n\nİçindeki Şefi Keşfet iRecipe, lezzetin sınırlarını zorlamak ve kendi mutfak serüveninizi başlatmak için tasarlandı. \nBinlerce eşsiz tarif, kolay kullanım ve yaratıcılığınızı ortaya çıkarmak için harika bir platform sunuyoruz. Uygulama Özellikleri: Geniş ve Çeşitli Tarifler: Ana yemekten tatlıya, çorbadan meze tariflerine kadar, iRecipe, her damak zevkine hitap eden geniş bir tarif koleksiyonuna sahiptir.Favorilere Ekle ve Kişiselleştir: Beğendiğiniz tarifleri favorilere ekleyerek, kişisel koleksiyonunuzu oluşturun. Kendi yemek tariflerinizi ekleyerek, bu büyüleyici dünyaya kendi lezzetinizi katın.Kategorize Edilmiş Tarifler: İhtiyacınıza uygun tariflere hızlıca ulaşın. Ana yemeklerden tatlı tariflerine kadar her kategori özenle düzenlenmiştir.Topluluk Tarifleri: Siz de mutfakta yaratıcılığınızı paylaşın! Kendi tariflerinizi ekleyin ve diğer iRecipe kullanıcılarıyla deneyimlerinizi paylaşın.iRecipe, yemek yapma deneyimini daha kişisel, ilginç ve lezzetli hale getirmek için burada. Herkesi kendi mutfak serüvenlerine katılmaya ve bu lezzet dolu yolculuğa çıkmaya davet ediyoruz!Afiyet olsun ve keşfetmeye başlayın!';

  @override
  String get privacyDescription => 'Gizlilik Politikası\n\nGizliliğinizi korumak iRecipe uygulaması olarak en önemli önceliklerimizden biridir.\nBu gizlilik politikası, iRecipe uygulamasının, kişisel bilgilerinizi nasıl topladığını, kullanmayı ne şekilde sınırladığını ve koruduğunu açıklamaktadır.\nLütfen bu politikayı dikkatlice okuyunuz.\n\nKişisel Bilgilerin Toplanması ve Kullanımı\niRecipe uygulamasını kullanırken, bazı kişisel bilgilerinizi toplayabiliriz.\nBu bilgiler, adınız, e-posta adresiniz, coğrafi konumunuz gibi tanımlayıcı bilgiler olabilir.\nBu bilgileri, size özel deneyimler sunmak, taleplerinizi yerine getirmek, iletişim kurmak ve hizmetlerimizi iyileştirmek amacıyla kullanabiliriz.\nAncak, kişisel bilgilerinizi üçüncü taraflarla paylaşmayız veya satmayız.\n\nÇerezler\niRecipe uygulaması, çerezleri kullanabilir.\nÇerezler, tarayıcınız tarafından bilgisayarınıza veya mobil cihazınıza kaydedilen küçük metin dosyalarıdır.\nBu çerezler, uygulamayı daha iyi anlamamıza ve size daha iyi bir kullanıcı deneyimi sunmamıza yardımcı olur.\nÇerezleri kabul etmek veya reddetmek konusunda tercihinizi belirleyebilirsiniz.\nAncak, bazı özelliklerin doğru çalışması için çerezleri etkinleştirmeniz gerekebilir.\n\nGüvenlik\nKişisel bilgilerinizin güvenliğini sağlamak için gerekli önlemleri alırız.\nAncak, internet üzerinden yapılan iletişimlerin veya elektronik depolama yöntemlerinin tamamen güvenli olmadığını unutmayınız.\nHerhangi bir bilgiyi bize iletmek veya uygulamayı kullanmak tamamen kendi sorumluluğunuzdadır.\n\nDiğer Web Sitelerine Bağlantılar\niRecipe uygulaması, diğer web sitelerine veya hizmetlere bağlantılar içerebilir.\nBu bağlantılar üzerinde kontrolümüz olmadığından, bu web sitelerinin veya hizmetlerin gizlilik uygulamalarından sorumlu değiliz.\nBu web sitelerini ziyaret etmeden önce ilgili gizlilik politikalarını okumanızı öneririz.\n\nDeğişiklikler\nBu gizlilik politikasını zaman zaman güncelleyebiliriz.\nHerhangi bir güncelleme olduğunda, değişikliklerin geçerli olacağı tarihi belirteceğiz.\nBu sayfayı düzenli olarak kontrol etmeniz, güncel politikaları takip etmeniz önemlidir.\n\nİletişim\nGizlilik politikamızla ilgili herhangi bir sorunuz veya endişeniz varsa, lütfen bize ulaşın.\nSize yardımcı olmaktan memnuniyet duyarız.\nBu gizlilik politikası, iRecipe uygulamasının gizlilik uygulamalarını açıklamaktadır.\nUygulamamızı kullanarak, bu politikayı kabul ettiğinizi beyan etmiş olursunuz.\n\nSon güncelleme tarihi: [Güncel tarih]';
}
